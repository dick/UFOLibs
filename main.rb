  ...
  configure do
	...
	helpers do
		...
		def ufo_search(description, location, size)
			results = Chimps::QueryRequest.new("science/astronomy/seti/nuforc/ufo_sightings_search", :query_params => { :description => description, :location => location, :size => size } ).get
